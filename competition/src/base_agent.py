...
class BaseAgent:
    ...
    def _register_with_server(self, force: bool = False, initial: bool = False) -> None:
        ...
        try:
            # The hosted Fly service can take 20+ seconds to answer during a
            # matchmaking wave even though DNS and TLS are healthy. Keep the
            # connect deadline short, but allow a slow registration response.
            r = requests.post(url, json=payload, timeout=(6, 45))
    ...
    def _join_queue(self) -> int:
        ...
        try:
            # The hosted server can commit the queue mutation before its HTTP
            # response reaches the client. Give it more room than ordinary API
            # calls, then confirm membership if the response itself times out.
            r = requests.post(
            ...
            try:
                status = requests.get(
                    f"{self.email_server_url}/queue_status", timeout=10
                ).json()
                waiting = list(status.get("agents_waiting") or [])
                if self.agent_id in waiting:
                    pos = waiting.index(self.agent_id) + 1
                    print(
                        f"[{self.agent_id}] Joined matchmaking queue "
                        f"(position {pos}; confirmed after slow response)"
                    )
                    return pos
            except Exception:
                pass
            raise RuntimeError(
                "join_queue response timed out and queue membership could not be confirmed"
            )
        ...
        if r.status_code == 409:
            print(f"[{self.agent_id}] Already queued or in a match - continuing")
            return -1
    ...
    def send_message(self, to_agent: str, subject: str, body: str) -> Dict:
        ...
        try:
            ...
            try:
                # The queued endpoint returns the same message id but avoids
                # serializing concurrent game traffic behind the synchronous
                # storage path. Bound both connection and response waits so one
                # slow acknowledgement cannot consume an entire round.
                ...
                # The server often commits the message before Fly's response
                # reaches this client (confirmed by the watch trajectory). Treat
                # that state as delivered-but-unconfirmed and continue processing
                # the rest of the round instead of blocking every later action.
